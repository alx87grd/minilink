...
| --- | --- | --- | --- |
| Value iteration / DP | `PlanningProblem`, `StateSpaceGrid`, `DynamicProgrammingPlanner`, `LookupTableController`, `plot_cost2go` / `plot_policy` | `pendulum_value_iteration`, `pendulum_value_iteration_vs_lqr`, `demos/planning/value_iteration/` | `final_time` reads `problem.tf`; `success` reports convergence; notebooks wire with `vi_ctl @ plant` |
...
**Status 2026-09-16:** Sphinx site is autodoc only (`docs/api/`); the five-slide
deck and `docs/roadmap.rst` are gone. The user guide is `examples/tutorial/`.
