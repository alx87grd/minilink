...
| Analysis / Lyapunov certificates | teaching (GRO860) | 6 | Landed 2026-09-11 from [lyapunov-certificates.md](docs/plans/lyapunov-certificates.md) steps L1–L6: `region_of_attraction` + `LyapunovCertificate` (`verify`, `plot`, `contains`), two `System` shortcuts, demo `analysis_region_of_attraction.py` (gentle vs stiff LQR: faster poles prove a 3x smaller region while the true basin is unchanged) and showcase §11 of `showcase_from_rl_to_bode.ipynb` (the twelve-state UR5 loop with the network inside), 13 tests including the reverse Van der Pol against its limit cycle. Quadratic `V` only; the level is a sampled estimate and the search reports `sample_limited` when its own halves disagree. | Maintainer review of the API (D1–D5); sum-of-squares method and discrete time are the named later steps. |
| Planning / RL planner (`reinforcement_learning/`) | research → provisional | 3 | Landed 2026-09-10 from the vision plan (steps R1–R5b): cost horizon/discount + problem exit rule, `StochasticPlanningProblem` + distributions, `MonteCarloEvaluator`, `NeuralPolicyController` + `MLP`, `ReinforcementLearningPlanner` with PPO (on-policy) and SAC (off-policy) as one-file algorithms; pendulum swing-up in 2 s (PPO) / 20k steps (SAC), drone learn-to-fly reproduced. Tests in `test_rl_planner.py`, `test_planning_stochastic.py`. | Official demos in `examples/demos/rl/` (pendulum, cart-pole, car, rocket, drone) match the experimental scripts; intro chapter `11_reinforcement_learning.ipynb`. Next: maintainer review of the demos and the notebook, then R7 (retire `experimental/ppo_jax` and its scripts); composite feature block (D3) open. |
...
| RL via `Sys2Gym` + SB3 | `Sys2Gym`, `SB3Controller`, `plot_control_law` | `drone_ppo_learn_to_fly`, `pendulum_swing_up_vi_vs_lqr_vs_ppo` | compiled `step`; PPO notebook trains to the same qualitative policy. The native path (`ReinforcementLearningPlanner`, intro `11_reinforcement_learning`, `demos/rl/`) landed 2026-09-10 as a **provisional** second entry; whether it replaces the SB3 path for the course is an open ruling (§6) |
...
- **Open (v0.2, draft 2026-09-11):** Lyapunov certificates in the analysis band — [docs/plans/lyapunov-certificates.md](docs/plans/lyapunov-certificates.md): `region_of_attraction(sys)` returning a `LyapunovCertificate` with `contains`, `verify` (Monte Carlo inside the certified set) and `plot`, plus the two `System` shortcuts. Same verb for LQR, DP, MPC and a learned policy. Decisions D1–D5: naming, whether the state set caps the level, where `verify` lives, discrete time, course placement.
- **Open (v0.2, landed provisional 2026-09-10):** RL as a planner — [docs/plans/rl-planner-vision.md](docs/plans/rl-planner-vision.md) (steps R1–R6 landed, R7 retire the prototype pending; ruling wanted on the course path: SB3 bridge, the native planner, or both): a stochastic problem class (`x0`, params and disturbance distributions, expected or worst-case criterion), an exit rule and horizon/discount semantics shared by trajopt, DP and RL (decisions D1–D2), the feature/policy block shape (D3), bare JAX vs Flax/Optax (D4), one `ReinforcementLearningPlanner` beside DP (D5), and a Monte Carlo evaluator as the second verb.
