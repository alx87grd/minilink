...
Borrow from the **readability** of the legacy Pyro toolbox where it helps: code should read like a **robotics textbook** plus a thin Python translation, not like generic application plumbing. This complements (does not replace) **public** typing and Sphinx depth rules above—contracts on user-facing APIs stay explicit; **internal** paths stay lean.

- **Mental model for objects**: A *system* is dynamics and outputs (`f`, `h`, ports, bounds); a *trajectory* is time with state/input (and outputs when relevant); *blocks* compose in diagrams; an optional *symbolic* layer feeds readable Python models; *compilation* produces flat evaluators (NumPy/JAX) that remain **recognizable** as the same equations—not buried behind opaque facades. Prefer names and layouts that make that pipeline obvious.
- **Pipeline story in the code**: The narrative **readable model → (optional symbolic) → simulate / optimize → compiled evaluator** should be easy to follow from folder and API names. Keep the middle “readable Python” layer the main teaching surface; push optimizer/solver glue to `optimization/`, `simulation/`, `compile/` without dragging notational clutter back into modeling.
- **Thin boundaries**: At layer edges, pass a **small vocabulary** of clear types (`Trajectory`, `System`, `PlanningProblem`, evaluators, transcribers). Signatures and short methods should read like lecture pseudocode (`roll out x`, `stack constraints`) rather than deep stacks of helpers.
- **Audience (MATLAB-adjacent, engineering students)**: When it clarifies the math, prefer **explicit loops and named temporaries** over dense one-liners; put dimensions and signal shapes in class/module docstrings; avoid Python idioms that hide indices or tensor shapes. The goal is a reader who thinks in state equations first, Python second.
- **Minimize distracting ceremony**: Avoid broad `try`/`except` in internal integrators, rollouts, and transcribers—let wrong types or shapes fail in the usual NumPy/SciPy/JAX way unless the surface is **public** or the failure mode is truly confusing. Do not scatter `np.asarray` / `np.array` through every helper if the contract is already `ndarray`; **normalize once** at public entry points when list-like inputs are allowed. Skip elaborate `typing` machinery (deep `Protocol` nests, nested `TypeVar` games) unless a stable public API needs it.
- **Imports—keep headers light**: Prefer the **shortest** import block that preserves clarity. **`from __future__ import annotations` is optional**: use it when it genuinely simplifies annotations (forward references to classes in the same module, heavy composite types) or matches an established file pattern; do **not** add it to every new file “by policy.” Avoid importing `typing` symbols that only shave off rare edge cases.
