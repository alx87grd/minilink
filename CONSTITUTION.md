...
1. **One toolbox, one model.** A student, engineer, or researcher never changes frameworks to
   ...
   modern control, trajectory optimization, and reinforcement learning.
...
4. **Product Boundaries (What Minilink is NOT):**
   ...
   - It is **not** a heavy multibody contact engine (it defines systems and wraps external physics when needed).
   - It is **not** an optimization modeling language or solver framework (optimization is a verb that operates on Systems).
   - It is the **integrating systems backbone** that unifies modeling, simulation, control, optimization, and learning under one clean, pythonic interface.

## 2. The Sacred Contract
...
3. **Compositional closure.** Wiring Systems together via operators (`@`, `>>`, `+`) yields a `System`.
   The set of Systems is algebraically closed under series, parallel, and feedback connections: a composite
   diagram compiles down to an identical pure continuous dynamical system ($\dot{X} = F(X, U, t; P)$).
   Any property or tool valid for a leaf is valid for a composite diagram.
...
Minilink bridges two distinct worlds with a single API:
...
2. **Compositional closure** (Diagrams must behave identically to leaves; wiring must close).
3. **Domain readability** (Textbook mathematics over programmatic abstraction).
4. **Single-path implementation** (Code deduplication is desirable, but never at the expense of readability or purity).
