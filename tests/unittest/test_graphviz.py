...
class TestGraphvizRendering(unittest.TestCase):
    def test_system_block_html_lists_input_and_output_ports(self):
        controller = PropController()
        html = get_system_block_html(controller, "ctl")
    ...
    def test_get_system_graphe_builds_dot_source(self):
        graph = get_system_graphe(Integrator())

        self.assertIsNotNone(graph)
        self.assertIn("Integrator", graph.source)
        self.assertIn("y", graph.source)

    def test_get_diagram_graphe_includes_boundary_ports_and_edges(self):
        ...
        self.assertIn("ctl:u:e -> plant:u:w", source)
        self.assertIn("plant:y:e -> output:y_meas:w", source)
