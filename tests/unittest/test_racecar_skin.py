"""Contract of the CAD meshes, the placement, the looks, and the plant that drives them."""
...
def test_the_meshes_live_inside_the_package():
    """Chassis, wheel and hinge meshes ship with the package."""
    assert ASSETS.parents[1].name == "graphical"
    assert ASSETS.parents[2].name == "minilink"
    assert MESH_DIR.is_dir()
    assert sorted(p.name for p in MESH_DIR.iterdir()) == sorted(STL_FILES)


def test_geometry_matches_the_plant():
    """Wheelbase, wheel radius and track are the 1/10 numbers."""
...
def test_the_lidar_sits_at_its_scan_height():
...
def test_frames_carry_the_whole_robot():
    """Placement returns the bicycle frames and every link, as valid poses."""
    frames = racecar_frames(1.0, 2.0, 0.3, 0.2)

    assert {"body", "axle_front"}.issubset(frames)
    assert set(LINKS).issubset(frames)
...
def test_both_front_wheels_turn_about_one_centre():
    ...
    for hub, angle in (([L, 0.5 * w], left), ([L, -0.5 * w], right)):
        arm = centre - np.asarray(hub)
        rolling = np.array([np.cos(angle), np.sin(angle)])
...
def test_the_hinges_are_turned_by_the_ackermann_angles():
    ...
    # Hinge and wheel joints use rpy 1.5708.
    for link, angle in (
...
@pytest.mark.parametrize("look", [racecar_skin_2d, racecar_skin_3d])
def test_every_look_draws_something_on_the_frames(look):
...
def test_the_3d_plant_places_its_own_skin():
    """``tf`` feeds the link frames from the state, wheel angles included."""
    car = UdeSRacecarDyn3D()
    x = np.zeros(car.n)
