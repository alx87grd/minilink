...
Historical note (relocated 2026-09-15). The prototype `minilink.experimental.ppo_jax` was retired (vision plan R7); training lives on `ReinforcementLearningPlanner`. Canonical scripts: `examples/demos/rl/` (pendulum swing-up, cart-pole, drone, rocket). A Lyapunov check of a learned pendulum law remains at `examples/experimental/rl/pendulum_rl_lyapunov_certificate.py`. The numbers below were measured on the prototype and still apply to the planner.

Notes from getting PPO to learn these plants (September 2026). Every number below was measured on a laptop CPU
...
The prototype became a planner:
...
mission scripts (pendulum swing-up, cart-pole, drone, rocket). Every lesson
below carries over: the exit rule is
