...
- [x] **Course notebooks off Google Drive (2026-09-15)** — four GRO860 exercises that still pointed to private Colab files now have minilink twins under `examples/teaching/`: `optimal_control/cartpole_lqr.ipynb` (infinite- and finite-horizon LQR; new `lqr_gain_schedule` / `lqr_finite_horizon` and `TimeVaryingStateFeedbackController`), `optimal_control/double_integrator_minimum_time_vi.ipynb`, `optimal_control/double_integrator_policy_evaluation.ipynb` (`PolicyEvaluator.value_at` / `plot_cost2go`, `StateSpaceGrid.plot_value`), `reinforcement_learning/function_approximation_sgd.ipynb` (plain numpy). The library module `policy_synthesis/approximation.py` stays. **Ex. 17.2** reverted the same day to the pyro Drive Colab: `cost_to_go_function_approximation.ipynb` was incorrect and was deleted. The notes link them; the course page needs no change.
