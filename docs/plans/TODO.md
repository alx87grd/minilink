...
- [x] **RL teaching ecosystem (2026-09-12)** — `teaching/reinforcement_learning/policy_gradient_to_ppo.ipynb` (the policy gradient to PPO, each formula checked against the planner's functions; `gae` exported from the RL package, now a registered teaching facade), `gymnasium_interface.ipynb` (the environment contract by hand, from a problem, trained through), native twins `pendulum_value_iteration_vs_lqr_vs_rl.ipynb` and `drone_ppo.ipynb`, tutorial 11 restructured around the planner API, and `demos/rl/pendulum_ppo_vs_sac_rl.py` promoted from `experimental/rl/`.
- [x] **Course notebooks off Google Drive (2026-09-15)** — GRO860 `\colab{}` targets live under `examples/teaching/courses/udes_gro860/` (renamed 2026-09-16). The library module `policy_synthesis/approximation.py` stays. The notes link them; the course page needs no change.
- [ ] **Approximate dynamic programming on the new bases** — fitted value iteration (`policy_synthesis/approximation.py` as the critic: sample states, Bellman targets with the model, `fit` or `sgd_step`, repeat) is the natural next notebook for the approximation chapter; design first, it touches the planner API.
- [ ] **S49 Retire the Stable-Baselines3 teaching notebooks** **[ask]**. GRO860 notes now link the native twins (`drone_ppo`, `pendulum_value_iteration_vs_lqr_vs_rl`). Delete `teaching/courses/udes_gro860/drone_ppo_sb3.ipynb` and `pendulum_value_iteration_vs_lqr_vs_ppo_sb3.ipynb`, their notebook overrides and allowlist entry, and the README rows marked as Stable-Baselines3 versions.
- [x] **S46 Promote the RL names to the teaching interface** **[ask — public names]** (2026-09-16). `StochasticPlanningProblem`, `ReinforcementLearningPlanner`, `NeuralPolicyController`, `MonteCarloEvaluator`, `Evaluation`, `Gaussian` / `Uniform` are on the root prelude and the `test_teaching_surface.py` registry; GRO860 notebooks and `demos/rl/` import them from the root. `angle_features` stays on `minilink.control` (a pendulum wrapping helper, not a core object). Gymnasium stays taught as the standard of the domain.
- [x] **S48 One result for every planner** (ruled 2026-09-15). `TrajectoryPlan`, `PolicyPlan`, `SolveMetadata` and `MonteCarloReport` are gone (clean break, deprecation note in ROADMAP §2); every planner returns a `PlanningSolution` (`policy`, `solver` record, `trajectory`, `evaluation`, `cost_to_go`, `success`), open-loop planners as a `TrajectorySource` (`interpolation="linear"` for trajopt, `"previous"` for RRT) so `policy >> plant` replays the plan; non-native fields are opt-in through `solve(evaluate=True, n_trials=)`; RL fills `cost_to_go` only when the training discount equals the cost's. `tests/unittest/test_planning_solution.py` checks every planner's policy against its trajectory.
- [x] **S47 Training log** (ruled 2026-09-15, default restored the same day). `ReinforcementLearningPlanner(verbose=True)` prints the environment's semantics and one line per update; pass `verbose=False` to silence it.
- [x] **RL module as a textbook, and the foundational algorithms (2026-09-15)** — `planning/reinforcement_learning/` rebuilt on domain objects: `StochasticPolicy` (the controller block plus an exploration head, one pytree `theta`, batch-aware `sample` / `log_prob` / `entropy` / `input`) replaces the callable bag; one `rollout` collector for both families, `advantages` (TD errors) + `gae`, `returns_to_go`; algorithms own their train state and declare their family (`on_policy`, `head_kind`, `critic_kind`, `episodic`); `RolloutEnvironment(backend=)` runs on NumPy too, so the NumPy Monte Carlo trials and the tabular learners share the exit-rule semantics. Environment, both Monte Carlo backends, PPO and SAC proven bit-identical by a seeded baseline. New: `REINFORCE`, `ActorCritic` (the ladder to PPO, course ch. 18–19), `TabularLearningPlanner` with `QLearning` / `SARSA` / `MonteCarloControl` and `EpsilonGreedy` / `UCB` on a `StateSpaceGrid` (course ch. 14–16; stochastic rounding makes the sampled MDP the one value iteration solves), demos `pendulum_q_learning_vs_value_iteration_rl.py`, `double_integrator_sarsa_vs_q_learning_rl.py`, `pendulum_reinforce_vs_actor_critic_vs_ppo_rl.py`, teaching notebook `from_value_iteration_to_ppo.ipynb`. Algorithm-ladder example files were deleted 2026-09-15 (library algorithms stay).
- [x] **R7 retire `experimental.ppo_jax` (2026-09-15)** — prototype module, unit test, and experimental proto scripts deleted; official demos are `examples/demos/rl/` (pendulum swing-up, cart-pole, drone, rocket); algorithm-ladder demos/notebooks dropped (library algorithms stay); tuning notes moved to [../reviews/2026-09-10-rl-tuning.md](../reviews/2026-09-10-rl-tuning.md).
- [ ] **S50 SAC actor step: pre-update or updated critic** **[ask]**. The actor loss is evaluated on the critic *before* its own Adam step (the module's original choice, kept for bit-identity); the reference implementations (SB3, CleanRL) evaluate it on the updated critic. One line in `algorithms/sac.py`; re-measure `pendulum_swing_up_rl.py` if changed.
- [ ] **S51 Policy iteration on the grid** (course ch. 12): alternate `PolicyEvaluator` sweeps and greedy improvement as a `DynamicProgrammingPlanner` mode or a sibling; the tabular SARSA-with-improving-policy story then has its model-based twin. **[ask — planner API]**
- [ ] **S52 Deep Q-learning** (course ch. 17): a discrete-action head over the grid's input levels, `QFunction` with replay and a target network, as one more `Algorithm`; needs a discrete-action policy object beside `StochasticPolicy`. Research lane first.
...
- [x] **S55 One solution for LQR, and `compare`** (landed 2026-09-17, plan `docs/plans/planning-solution-comparison.md`). `PlanningSolution` carries its `problem`; `LQRPlanner` returns the Riccati design as a solution with `cost_to_go`; solution verbs `plot_control_law` / `plot_cost_to_go` / `plot_trajectory` / `plot_cost` and planner shortcuts; `compare(...)` table and overlays; evaluators accept a solution for its policy. Open: `cost_to_go` as a `Field` (phase 4), solver-specific views on the records (maintainer), the demo and notebook rewrites (maintainer, drafts in the plan).
- [x] **S56 One horizon** (ruled 2026-09-17). `CostFunction.horizon` removed; `problem.tf` alone decides `horizon_kind()`, and `h` is charged exactly when `tf` is finite. An infinite-horizon problem run for a while is `episode_length` on the evaluator / RL planner. Reason: trajopt and DP already read `tf` and charged `h` unconditionally, so a cost declaring `"infinite"` on a finite `tf` scored differently per tool.
...
- [ ] **S57** Workspace geometry module **[ask — core]**. Designed 2026-09-21: [geometry-module.md](geometry-module.md). Package `core/geometry/` (shapes, Path, Track, Scene, probes, `bind`, spatial Fields, `sample_grid`, catalog `oval_circuit` / `racecar_circuit` / `holonomic_forest`); shaping next to `Field.as_cost`; plots/overlays under `graphical/`; `planning.spatial` re-exports then delete in the same change; PurePursuit takes a Path/Track. Done when the UdeS racecar trio and the RRT forest demo use the catalog, control imports geometry with no planning import, and the suite is green. Glyph rename stays S30.
...
- [ ] **Core math objects** — `core-objects-4-fields.md` … `core-objects-6-demos.md` (phases 1–3 applied 2026-09-15, plan docs deleted; remaining steps name-preserving for the GRO860 notebooks, schedule at the maintainer's call):
  - [ ] Phase 1 governance edits **[ask — CONSTITUTION / RULES / AGENTS / DESIGN / ROADMAP]**: the object list and "one signature" in CONSTITUTION §2, RULES 2.3 / 2.5 / 4.3 / 5.1 / 5.4 / 5.17 / 5.21 / 6.1 / 6.10–6.12, AGENTS ask-first list, DESIGN §4 / §6, `examples/README.md` demo-style line.
  - [x] Phase 2 (landed 2026-09-15) `core/sets.py` textbook pass; `VectorSignal.box` **[ask]**; distributions → `core/distributions.py` **[ask]**; one `sample(key, n=None)` convention and `mean` attribute **[ask]**; `Set.bounding_box()` **[ask]** + the set-probe ratchet test; `StateSpaceGrid.X` / `.U`.
  - [x] Phase 3 (landed 2026-09-15) `X` unconstrained unless declared, an exit a failure at `infeasible_cost` everywhere, `on_exit` retired, the RL `training_zone`, the grid owns its domain; 21 example files declare `X=plant.state.box`.
  - [ ] Phase 4 `core/fields.py` **[ask]**: `Field` on `(x, u, t)` with `as_constraint` / `as_input_constraint` / `as_cost`, `QuadraticField` (Lyapunov `V`, `S(t)` from the Riccati sweep), `GridField` (DP tables, tabular `Q`), `CallableField` (RL critics), `LinearApproximator` as a field; spatial field subclasses live in `core/geometry/` ([geometry-module.md](geometry-module.md)); CBF plan amended.
  - [ ] Phase 6 demos to the rule: native plots/prints the demos hand-roll (agent lane), the flat-demo ratchet in `test_teaching_imports.py`, then the sweep file by file **[ask per file]**.
- [ ] Later ideas: vehicle view ports (`pose` / `bodyvel` on `DynamicBicycle` for impedance/PID); scene params / `J(z, p)` bind (DESIGN §4 planning-params pipeline B); `SolverFactory` ([optimizer-parametric-wiring.md](optimizer-parametric-wiring.md)); `MjxPlant`; Pacejka; stochastic forcing; neural MLP; ROS2 / FMI; sparse long-horizon trajopt; parametric `Shape` / `Set` / `Cost` / `Field` overrides (the "one signature" gap, core-objects-5-later-nouns.md); `Gaussian(cov=)` + `log_prob` and the disturbance convention (per-step covariance vs spectral density) before `estimation/`; sets and distributions over parameter dictionaries; `NoiseSource(distribution)`; `UnionSet`; `PlanningProblem.hamiltonian()` if the course teaches Pontryagin; trajectory post-filter; RRT-Connect; shared RNEA serial-chain stack; ABA on other RNEA arms.
...
- [ ] GMC714 modelling ladder: manipulators + the four-rung vehicle ladder as a `02_dynamics` lesson; robotic PID wrappers; trajectory LQR landed 2026-09-15 (`trajectory_lqr` / `TrajectoryFeedbackController`, `demos/control/trajectory_lqr_cartpole.py`, section 6 of `teaching/courses/udes_gro860/cartpole_lqr.ipynb`).
