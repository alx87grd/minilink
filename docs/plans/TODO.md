...
- [ ] **Core math objects** — `core-objects-4-fields.md` … `core-objects-6-demos.md` (phases 1–3 applied 2026-09-15, plan docs deleted; remaining steps name-preserving for the GRO860 notebooks, schedule at the maintainer's call):
  ...
  - [ ] Phase 4 `core/fields.py` **[ask]**: `Field` on `(x, u, t)` with `as_constraint` / `as_input_constraint` / `as_cost`, `QuadraticField` (Lyapunov `V`, `S(t)` from the Riccati sweep), `GridField` (DP tables, tabular `Q`), `CallableField` (RL critics), `LinearApproximator` as a field; CBF plan amended.
