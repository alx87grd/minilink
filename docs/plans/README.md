...
| [core-objects-1-governance.md](core-objects-1-governance.md) | Vision, constitution and rules: the mathematical objects named, one signature `(x, u, t; p)`, lower late, the demo rule, the amendments and rulings of 2026-09-15 | Core contract — applied 2026-09-15 |
| [core-objects-2-sets-distributions.md](core-objects-2-sets-distributions.md) | `core/sets.py` textbook pass, `.box` on ports, distributions to `core/`, one draw convention, `Set.bounding_box()` and its ratchet test, the grid's boxes | Core contract — applied 2026-09-15 |
| [core-objects-3-exit-rule.md](core-objects-3-exit-rule.md) | `X` a hard constraint, unconstrained unless declared; `on_exit` retired, `exit_cost` → `infeasible_cost` with a derived announced default; the RL `training_zone`; the grid owns its domain | Core contract — applied 2026-09-15 |
