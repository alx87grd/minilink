...
- [**`cartpole_lqr.ipynb`**](optimal_control/cartpole_lqr.ipynb): LQR from the linearized cart-pole — the algebraic Riccati equation on an infinite horizon, the Riccati differential equation and a gain schedule on a finite one, the same design along a swing-up trajectory planned by direct collocation, and the backward Euler recursion written by hand.
