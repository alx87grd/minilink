...
def _build_hybrid(*, maxiter: int = 50, step_disp: bool = False):
    ...
    planner = TrajectoryOptimizationPlanner(
        ...
        optimizer_options={"maxiter": maxiter, "ftol": 1.0},
    )
    ...
    hybrid = mpc @ sys
    return hybrid, x0, mpc, planner


def run_smoke(*, tf: float = 1.0) -> None:
    """Headless demo check: short hybrid MPC rollout."""
    hybrid, x0, _, _ = _build_hybrid(maxiter=20, step_disp=False)
...
def main() -> None:
    hybrid, x0, mpc, planner = _build_hybrid(maxiter=50, step_disp=STEP_DISP)
