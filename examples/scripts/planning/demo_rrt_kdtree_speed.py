"""Compare RRT nearest-neighbour backends on the same holonomic obstacle problem.

Run from repo root::

    python examples/scripts/planning/demo_rrt_kdtree_speed.py

The scene, extender, and ``max_nodes`` budget are shared. Only
``RRTOptions.nearest_backend`` changes between ``brute_force`` and ``kd_tree``
(Euclidean L2 via SciPy ``cKDTree``). RRT* uses ``optimize_after_goal=True`` with
``convergence_patience=2000`` so the tree keeps growing after the first goal
connection — typically several thousand nodes on this scene. Baseline RRT still
stops at the first goal hit, so its trees stay smaller. Add more ``SEEDS`` for
a statistical comparison (each extra seed adds ~2 minutes for RRT* on this scene).
"""
...
def make_options(planner_cls, seed, nearest_backend):
...
def run_planner(planner_cls, seed, nearest_backend):
    planner = planner_cls(
        problem,
        extender=extender,
        options=make_options(planner_cls, seed, nearest_backend),
    )
...
for seed in SEEDS:
    ...
    for label, planner_cls in planner_pairs:
        for backend in backends:
            row[(label, backend)] = run_planner(planner_cls, seed, backend)
