...
class DynamicBicycleRearWheelDrive(DynamicSystem):
    """
    ...
    def generalized_d(
        ...
        c_d, s_d = np.cos(delta), np.sin(delta)
        Fx_f_b = Fx_front * c_d - Fy_front * s_d
        Fy_f_b = Fx_front * s_d + Fy_front * c_d
...
class DynamicBicycleRearWheelDriveEngine(DynamicBicycleRearWheelDrive):
    ...
    def __init__(self):
        ...
        self.rpm = np.linspace(rpm_min, rpm_max, resolution)
        self.engine_torque_map = max_torque_curve(self.rpm)
    ...
    def engine_torque_from_throttle(self, v: np.ndarray, throttle: float):
        ...
        rpm = abs(w_rear) * self.transmission_ratio * 60.0 / (2.0 * np.pi)

        # Clamp RPM inside map range
        rpm = np.clip(rpm, self.rpm[0], self.rpm[-1])

        # Max engine torque from map
        max_engine_torque = np.interp(rpm, self.rpm, self.engine_torque_map)

        # Clamp throttle
        throttle = np.clip(throttle, 0.0, 1.0)

        # Engine torque
        engine_torque = throttle * max_engine_torque

        # Convert engine torque to rear wheel torque
        tau_rear = engine_torque * self.transmission_ratio
    ...
    #     tau_rear = self.engine_torque_from_throttle(v, throttle)

    #     u_drive = np.array([tau_rear, delta], dtype=float)
    ...
    def get_kinematic_transforms(self, x: np.ndarray, u: np.ndarray, t: float):
        ...
        Fx_f, Fy_f, Fx_r, Fy_r = self.compute_tire_physics(vb, u_drive)
