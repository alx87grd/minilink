...
def zeros(A, B, C, D):
    ...
    if n == 0 or m != C.shape[0]:
        return np.array([], dtype=complex)
    pencil = np.block([[A, B], [C, D]])
    descriptor = np.zeros_like(pencil)
    descriptor[:n, :n] = np.eye(n)
    values = linalg.eigvals(pencil, descriptor)
...
def root_locus(A, B, C, D, gains=None, *, n=400):
    ...
    A, B, C, D = _matrices(A, B, C, D)
    radius = max(
        np.max(np.abs(np.concatenate([poles(A), zeros(A, B, C, D)])), initial=0.0), 1.0
    )
    if gains is None:
        k_max = _gain_reaching(A, B, C, D, 10.0 * radius)
    ...
    gains, roots = list(gains), roots

    # Refine where a branch jumps: insert the midpoint gain and match again.
    step = 0.02 * radius
    k = 0
    while k < len(gains) - 1:
        if (
