...
class TrajectoryOptimizationBenchmarkConfig:
    """Shared grid and optimizer defaults for a trajopt benchmark run."""
...
def jax_trajopt_available() -> bool:
    """Return whether JAX trajectory-optimization benchmark variants can run."""
    if not HAS_JAX_TRAJOPT:
        return False
    try:
...
def default_trajectory_optimization_variants(
    *,
    ...
    """Return the default trajopt benchmark sweep."""
    variants = [
        TrajectoryOptimizationBenchmarkVariant(
...
def _build_planner(
    ...
    elif variant.transcription == "multiple_shooting":
        if variant.compile_backend == "jax":
    ...
    elif variant.transcription == "collocation":
        if variant.compile_backend == "jax":
...
def _run_once(
    ...
    planner = _build_planner(variant, config)
    ...
    max_eq, min_ineq, max_bound = _constraint_metrics(program, result.z)
...
def benchmark_trajectory_optimization(
    ...
    rows = tuple(
        _summarize(
            variant,
            [_run_once(variant, config, warm_guess) for _ in range(config.n_runs)],
        )
        for variant in variants
    )
...
def print_trajectory_optimization_benchmark(
    ...
    rows = result.rows
    name_width = max([21, *(len(row.variant.name) for row in rows)])
    transcription_width = max([13, *(len(row.variant.transcription) for row in rows)])
    start_width = max([5, *(len(row.variant.start) for row in rows)])
    rule_width = name_width + transcription_width + start_width + 126
    ...
    print("-" * rule_width)
    print(
        f"{'variant':<{name_width}} {'transcription':<{transcription_width}} "
        f"{'start':<{start_width}} {'backend':<7} {'deriv':<11} {'prec':<4} "
        f"{'opt':<7} {'method':<12} "
    ...
    for row in rows:
        ...
        print(
            f"{variant.name:<{name_width}} "
            f"{variant.transcription:<{transcription_width}} "
            f"{variant.start:<{start_width}} {variant.compile_backend:<7} "
