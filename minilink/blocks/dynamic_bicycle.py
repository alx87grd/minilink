...
class DynamicBicycle(DynamicSystem):
    ...
    def generalized_d(self, q: np.ndarray, v: np.ndarray, u_in: np.ndarray) -> np.ndarray:
    ...
    def f(self, x: np.ndarray, u: np.ndarray, t: float = 0.0, params=None) -> np.ndarray:
        q = x[0:3]
        v = x[3:6]
        u_in = np.array([self.u2input_signal(u, "w_rear")[0], self.u2input_signal(u, "delta")[0]])
    ...
    def h(self, x: np.ndarray, u: np.ndarray, t: float = 0.0, params=None) -> np.ndarray:
        return x.copy()
...
class DynamicBicycleCar3D(DynamicBicycle):
    ...
    def get_kinematic_geometry(self):
        ...
        tire_rad = max(
            0.045, self._visual_tire_radius_ratio * min(self.r_f, self.r_r)
        )
