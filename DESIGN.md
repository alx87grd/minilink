...
| `state` | `state.py` | full state `x`; constant gain `K` (`StateFeedbackController`, what `lqr` returns) or a gain schedule `K(t)` interpolated between samples and held or replaced past its end (`TimeVaryingStateFeedbackController`, what `lqr_finite_horizon` returns; the schedule itself comes from `lqr_gain_schedule`, the Riccati differential equation propagated exactly through the Hamiltonian system) | `(x0, x1)` |
