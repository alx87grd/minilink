...
`sys.show(x,u,t)` vs `sys.show_v2(x,u,t)`.
...
Per-batch: `show_v2()` pixel-identical to `show()` for the parity plants; full
pytest green; Phase 0 baselines unchanged.

## User Review 3a–3e

`sys.show(x,u,t)` vs `sys.show_v2(x,u,t)` side-by-side per batch.
