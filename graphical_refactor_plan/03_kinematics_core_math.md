...
| `apply(T, pts)` | points | homogeneous transform p′ = T·p̃ over a batch (relocated from `planning/spatial/robot.py`) |
...
- **Collision (Phase 7)** — `RobotBody`/`PlanarRigidBody.body_poses` converges onto
  ...
  chassis and the clearance probes. `apply` moves here from
