...
todos:
  - id: p0-baseline
    content: "Phase 0: PNG baseline script (~12 plants x 3 samples) + pytest harness on old pipeline"
    status: pending
  - id: p1-foundation
...
| Phases 3–4 | `show_v2()` pixel-identical to `show()` per migrated plant |
...
All phases **pending**. Start with **Phase 0** on a clean feature branch.
